...
constexpr uint64_t kDefaultTotalBytes = 4ULL * 1024ULL * 1024ULL * 1024ULL;
...
constexpr uint64_t kDesiredNonL0Runs = 3;
constexpr auto kShapePollInterval = std::chrono::seconds(1);
...
void PrintUsage(const char* argv0) {
  std::cout
      ...
      << "  --total_bytes=N               Final logical KV bytes after updates.\n"
...
std::optional<int> FindNonL0LevelWithAtLeastRuns(
    const std::vector<LevelStructureSummary>& summary, uint64_t min_runs) {
  for (const auto& level : summary) {
    if (level.level > 0 && level.run_count >= min_runs) {
      return level.level;
    }
  }
  return std::nullopt;
...
void VerifyPointLookups(DB* db, uint64_t key_count, uint64_t sample_count,
  ...
  const uint64_t step =
      ChoosePermutationStep(key_count, seed ^ 0x9e3779b97f4a7c15ULL);
...
void VerifyRangeLookups(DB* db, uint64_t key_count, uint64_t sample_count,
  ...
  ReadOptions read_options;
  const uint64_t step =
      ChoosePermutationStep(key_count, seed ^ 0xd1b54a32d192ed03ULL);
...
int main(int argc, char** argv) {
  ...
  const uint64_t total_written_bytes = logical_bytes;
  ...
  std::cout << "  final logical bytes: " << logical_bytes << " ("
            << FormatBytes(logical_bytes) << ")" << std::endl;
  std::cout << "  total written bytes: " << total_written_bytes << " ("
            << FormatBytes(total_written_bytes) << ")" << std::endl;
  ...
  std::optional<int> target_level;
  const auto shape_deadline = std::chrono::steady_clock::now() + kShapeWaitTimeout;
  while (true) {
    live_files.clear();
    db->GetLiveFilesMetaData(&live_files);
    target_level =
        FindNonL0LevelWithAtLeastRuns(SummarizeStructure(live_files),
                                      kDesiredNonL0Runs);
    if (target_level.has_value()) {
      std::cout << "  reached target shape at L" << *target_level << " with >= "
                << kDesiredNonL0Runs << " runs" << std::endl;
      break;
    }
    if (std::chrono::steady_clock::now() >= shape_deadline) {
      Die("timed out waiting for a non-L0 level to reach at least " +
          std::to_string(kDesiredNonL0Runs) + " sorted runs");
    }
    std::this_thread::sleep_for(kShapePollInterval);
  }

  status = db->SetOptions({{"disable_auto_compactions", "true"}});
  if (!status.ok()) {
    Die("SetOptions(disable_auto_compactions=true) failed: " +
        status.ToString());
  }

  live_files.clear();
  db->GetLiveFilesMetaData(&live_files);
  const bool saw_compaction = std::any_of(
      live_files.begin(), live_files.end(), [](const LiveFileMetaData& file) {
  ...
  std::cout << "Write throughput: "
            << (write_seconds > 0.0
                    ? FormatBytes(static_cast<uint64_t>(total_written_bytes /
                                                       write_seconds))
                    : "inf")
            << "/s" << std::endl;
